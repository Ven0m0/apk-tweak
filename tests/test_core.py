...
class TestDispatchHooks:
    ...
    def test_dispatch_multiple_hooks(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_dispatch_hook_error_handling(
...
class TestRunPipeline:
    ...
    def test_pipeline_creates_work_dir(
        self, sample_apk: Path, tmp_path: Path
    ) -> None:
    ...
    def test_pipeline_unknown_engine_skipped(
        ...
        with patch("rvp.core.get_engines") as mock_get:
            ...
            with caplog.at_level(logging.INFO):
                run_pipeline(
                    sample_apk, output_dir, ["unknown_engine"]
                )
    ...
    def test_pipeline_engine_error_propagates(
        self, sample_apk: Path, tmp_path: Path
    ) -> None:
        ...
        with patch("rvp.core.get_engines", return_value=engines):
            with patch("rvp.core.load_plugins", return_value=[]):
                with pytest.raises(RuntimeError):
                    run_pipeline(
                        sample_apk, output_dir, ["failing"]
                    )

    def test_pipeline_options_passed(
        self, sample_apk: Path, tmp_path: Path
    ) -> None:
