...
class TestValidateApkPath:
  ...
  def test_directory_instead_of_file(self, tmp_path: Path) -> None:
    ...
    apk_dir.mkdir()
    with pytest.raises(ValidationError, match="APK path is not a file"):
...
class TestValidateEngineNames:
  ...
  def test_all_valid_engines(self) -> None:
    ...
    unknown = validate_engine_names(["revanced", "lspatch"], available)
    assert unknown == []

  def test_some_invalid_engines(self) -> None:
    """Test validation with some invalid engines."""
    available = {"revanced": None, "lspatch": None}
    unknown = validate_engine_names(["revanced", "unknown", "invalid"], available)
