...
def test_run_command_scrubs_sensitive_args(mock_context):
  """Test that sensitive arguments are scrubbed from logs."""
  cmd = [
    "java",
    "-jar",
    "tool.jar",
    "--keystore-password",
    "supersecret",
    "--keystore-entry-password=alsosecret",
    "--password",
    "pwd123",
    "-p",
    "pwd456",
  ...
  # The command shouldn't fail or timeout, we just want to verify logging
  # Let's mock subprocess.run so it returns a successful completed process
  import subprocess
  from unittest.mock import patch

  with patch("subprocess.run") as mock_run:
    mock_run.return_value = subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    run_command(cmd, mock_context, check=False)
  ...
  # Find the EXEC log
  exec_log = next((log for log in logs if log.startswith("EXEC:")), None)
  assert exec_log is not None, "EXEC log not found"

  # Verify sensitive data is scrubbed
  assert "supersecret" not in exec_log
  assert "alsosecret" not in exec_log
  assert "pwd123" not in exec_log
  assert "pwd456" not in exec_log
  assert "tok123" not in exec_log
  assert "key123" not in exec_log

  # Verify placeholders are present
  assert "--keystore-password ***" in exec_log
  assert "--keystore-entry-password=***" in exec_log
  assert "--password ***" in exec_log
  assert "-p ***" in exec_log
  assert "--token ***" in exec_log
  assert "--key ***" in exec_log

  # Verify normal args are untouched
  assert "--normal-arg normal_value" in exec_log
