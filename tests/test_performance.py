...
def test_engine_discovery_performance() -> None:
  ...
  assert engines1 is engines2, "Caching not working - returned different objects"
...
def test_plugin_loading_performance() -> None:
  ...
  assert plugins1 is plugins2, "Caching not working - returned different objects"
...
def test_magisk_zip_performance(tmp_path: Path) -> None:
  ...
  assert duration < 1.0, f"Magisk module creation took too long: {duration:.3f}s"
...
def test_context_metadata_access() -> None:
  ...
  assert write_duration < 0.01, f"Metadata writes too slow: {write_duration:.3f}s"
