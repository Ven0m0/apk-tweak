...
**APK Tweak (ReVanced Pipeline)** - An extensible Python pipeline for
APK modifications supporting ReVanced, LSPatch, DTL-X, and media optimization engines.
...
**Auto-Discovery**: Engines/plugins are discovered via
`pkgutil.iter_modules()` in `core.py`
...
- Examples: `feat(revanced): add multi-patch support`,
...
This runs: ruff, yamlfmt, yamllint, prettier, markdownlint, shfmt,
shellcheck, taplo, actionlint
...
External tools (for engines): `revanced-cli`, `java`, `pngquant`,
`jpegoptim`, `ffmpeg`
