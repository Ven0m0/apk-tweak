...
try:
  ...
  def _load_json(file_handle):
    return orjson.loads(file_handle.read())

  def _dump_json(data, file_handle):
    A = orjson.dumps(data, option=orjson.OPT_INDENT_2 | orjson.OPT_SORT_KEYS)
    file_handle.write(A.decode(_C))
except ImportError:
  import json
  from typing import Any
  ...
  def _dump_json(data, file_handle):
    json.dump(data, file_handle, indent=2, sort_keys=_B)
...
class Config:
  ...
  dtlx_analyze: bool = _A
  dtlx_optimize: bool = _A
  ...
  revanced_optimize: bool = _B
  revanced_debloat: bool = _B
  revanced_minify: bool = _B
  revanced_patch_ads: bool = _A
  ...
  rkpairip_apktool_mode: bool = _A
  rkpairip_merge_skip: bool = _A
  rkpairip_dex_repair: bool = _A
  rkpairip_corex_hook: bool = _A
  rkpairip_anti_split: bool = _A
  ...
  def load_from_file(B, path):
    A = path
    if not A.exists():
      raise FileNotFoundError(f"Config file not found: {A}")
    with open(A, encoding=_C) as C:
      D = _load_json(C)
      return B(**D)

  def save_to_file(A, path):
    from dataclasses import asdict as B

    path.parent.mkdir(parents=_B, exist_ok=_B)
    with open(path, "w", encoding=_C) as C:
      D = B(A)
      _dump_json(D, C)
