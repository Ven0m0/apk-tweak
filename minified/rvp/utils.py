...
def run_command(cmd,ctx,cwd=None,check=_A,timeout=None):
	...
	try:
		with subprocess.Popen(B,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=_A,cwd=cwd,bufsize=8192,encoding='utf-8',errors='replace')as C:
			if C.stdout:
				D=[]
				for J in C.stdout:
					I=J.strip()
...
def clone_repository(url,target_dir,ctx,timeout=TIMEOUT_CLONE):
	E=timeout;D=False;B=target_dir;A=ctx
	if B.exists():A.log(f"Repository already cloned at {B}, skipping");return _A
	A.log(f"Cloning repository from {url}")
	try:subprocess.run(['git','clone',url,str(B)],capture_output=_A,text=_A,timeout=E,check=_A);A.log(f"Repository cloned successfully to {B}");return _A
	except subprocess.TimeoutExpired:A.log(f"ERR: Clone timed out after {E}s");return D
	except subprocess.CalledProcessError as C:A.log(f"ERR: Clone failed: {C.stderr or C.stdout}");return D
	except OSError as C:A.log(f"ERR: Clone error: {C}");return D
