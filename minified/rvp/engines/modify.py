...
def _sign_apk(ctx,unsigned_apk,signed_apk):
	F=signed_apk;A=ctx;A.log('modify: Signing APK');C=cast(dict[str,Any],A.options.get(_D,{}));B=Path(str(C.get('path',A.work_dir/'keystore.jks')));G=str(C.get('alias','key0'));D=str(C.get('password','android'))
	if not B.exists():
		A.log(f"modify: Creating new keystore at {B}");H=[_E,'-genkey','-v','-keystore',str(B),'-alias',G,'-keyalg','RSA','-keysize','2048','-validity','10000','-storepass',D,'-keypass',D,'-dname','CN=APK Modifier, OU=Dev, O=APK, L=City, S=State, C=US']
		try:run_command(H,A)
		except(subprocess.SubprocessError,OSError)as E:A.log(f"modify: Keystore creation failed: {E}");return _A
	I=[_F,'sign','--ks',str(B),'--ks-key-alias',G,'--ks-pass',f"pass:{D}",'--out',str(F),str(unsigned_apk)]
	try:run_command(I,A,timeout=TIMEOUT_PATCH);return F.exists()
	except(subprocess.SubprocessError,OSError)as E:A.log(f"modify: Signing failed: {E}");return _A
def run(ctx):
	J='modify';A=ctx;A.log('modify: starting APK modifier');D=require_input_apk(A);K=[_C,'java',_E,_F];L,M=check_dependencies(K)
	if not L:raise RuntimeError(f"modify: Missing dependencies: {', '.join(M)}\nInstall with: apt-get install -y apktool default-jdk apksigner\nOr on Arch: pacman -S android-tools jdk-openjdk")
	E=A.work_dir/J;E.mkdir(parents=_B,exist_ok=_B);B=E/'decompiled';I=E/f"{D.stem}.unsigned.apk";C=A.output_dir/f"{D.stem}.modified.apk"
	if not _decompile_apk(A,D,B):raise RuntimeError('modify: Decompilation failed')
	F=A.options.get('modify_icon')
	if F:N=Path(cast(str,F));_modify_icon(A,B,N)
	...
	if not _recompile_apk(A,B,I):raise RuntimeError('modify: Recompilation failed')
	if not _sign_apk(A,I,C):raise RuntimeError('modify: Signing failed')
	A.set_current_apk(C);O=cast(dict[str,Any],A.options.get(_D,{}));P=str(O.get('path','auto-generated'));A.metadata[J]={'icon_modified':bool(F),'url_replaced':bool(G and H),'signed_apk':str(C),'keystore':P};A.log(f"modify: APK modification complete → {C}")