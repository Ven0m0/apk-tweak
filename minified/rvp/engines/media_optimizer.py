...
def _get_tool_availability(ctx):
	B=[_C,_D,_E];D,A=check_dependencies(B);C={B:B not in A for B in B}
	if A:ctx.log(f"media_optimizer: missing tools: {', '.join(A)}");ctx.log('media_optimizer: install via package manager (Arch: pacman -S pngquant jpegoptim ffmpeg)')
	return C
def _optimize_png(ctx,png_path,quality='65-80'):
	A=png_path
	try:B=subprocess.run([_C,_I,quality,'--ext',_F,_J,str(A)],capture_output=_B,text=_B,timeout=30,check=_A);return B.returncode==0
...
def _optimize_png_worker(png_path,quality='65-80'):
	A=png_path
	try:B=subprocess.run([_C,_I,quality,'--ext',_F,_J,str(A)],capture_output=_B,text=_B,timeout=30,check=_A);return A,B.returncode==0
	except(subprocess.TimeoutExpired,Exception):return A,_A
def _optimize_jpg(ctx,jpg_path,quality=85):
	A=jpg_path
	try:B=subprocess.run([_D,f"--max={quality}",_K,str(A)],capture_output=_B,text=_B,timeout=30,check=_A);return B.returncode==0
...
def _optimize_jpg_worker(jpg_path,quality=85):
	A=jpg_path
	try:B=subprocess.run([_D,f"--max={quality}",_K,str(A)],capture_output=_B,text=_B,timeout=30,check=_A);return A,B.returncode==0
	except(subprocess.TimeoutExpired,Exception):return A,_A
def _optimize_audio(ctx,audio_path,output_path,bitrate='96k'):
	...
	try:
		...
		E=subprocess.run([_E,'-i',str(A),_N,D,'-b:a',bitrate,'-y',str(B)],capture_output=_B,text=_B,timeout=60,check=_A)
...
def _optimize_audio_worker(audio_path,bitrate='96k'):
	...
	try:
		...
		B=A.with_suffix(A.suffix+'.tmp');E=subprocess.run([_E,'-i',str(A),_N,D,'-b:a',bitrate,'-y',str(B)],capture_output=_B,text=_B,timeout=60,check=_A)
...
def _repackage_apk(ctx,extract_dir,output_apk):
	...
	try:
		F={_F,'.jpg','.jpeg','.gif','.webp',_G,_H,'.mp4','.so','.ttf','.woff','.woff2'}
...
def _process_images(ctx,extract_dir,tools):
	L=tools;K='jpg';J='png';E=extract_dir;A=ctx;C={J:0,K:0};F=list(E.rglob('*.png'));G=list(itertools.chain(E.rglob('*.jpg'),E.rglob('*.jpeg')));A.log(f"media_optimizer: found {len(F)} PNG, {len(G)} JPEG files");H=L.get(_C,_A);I=L.get(_D,_A)
	if not H and not I:A.log('media_optimizer: no optimization tools available, skipping image optimization');return C
	Q=os.cpu_count()or 1;M=min(Q,8);A.log(f"media_optimizer: optimizing images with {M} shared workers")
	with ProcessPoolExecutor(max_workers=M)as N:
		D={}
		if H and F:
			for O in F:B=N.submit(_optimize_png_worker,O);D[B]=J,O
		if I and G:
			for P in G:B=N.submit(_optimize_jpg_worker,P);D[B]=K,P
		for B in as_completed(D):
			R,S=D[B];S,T=B.result()
			if T:C[R]+=1
	if not H:A.log('media_optimizer: pngquant not available, skipped PNG optimization')
	if not I:A.log('media_optimizer: jpegoptim not available, skipped JPEG optimization')
	A.log(f"media_optimizer: optimized {C[J]} PNG, {C[K]} JPEG files");return C
def _process_audio(ctx,extract_dir,tools):
	D=extract_dir;A=ctx
	if not tools.get(_E,_A):A.log('media_optimizer: ffmpeg not available, skipping audio optimization');return 0
