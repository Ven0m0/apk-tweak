...
def _remove_debug_symbols(ctx: Context, extract_dir: Path) -> int:
    ...
    removed_count = 0
    debug_patterns = [
        r".*\.map$", r".*\.log$", r".*proguard.*", r".*mapping\.txt$",
        r".*debug.*", r".*Debug.*", r".*/tests?/.*", r".*/test.*"
    ]
...
def _remove_locale_resources(ctx: Context, extract_dir: Path, keep_locales: list[str]) -> int:
